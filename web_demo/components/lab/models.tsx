    ...
    <group>
      <RoundedBox args={[1.6, 3, 1.6]} radius={0.07} smoothness={4} castShadow receiveShadow>
        <meshPhysicalMaterial
          ...
          emissiveIntensity={alarm ? 0.28 : 0}
          clearcoat={0.25}
      ...
      <RoundedBox args={[1.4, 2.74, 0.1]} radius={0.05} position={[0, 0.05, 0.8]} castShadow>
        <meshPhysicalMaterial
          ...
          metalness={0.68}
          roughness={0.24}
          emissive={alarm ? "#ff3b5c" : emissive}
          emissiveIntensity={alarm ? 0.32 : 0.08}
          clearcoat={0.35}
          clearcoatRoughness={0.16}
        />
      </RoundedBox>
      <Text position={[-0.05, 0.78, 0.875]} fontSize={0.13} color={alarm ? "#ffd0d8" : "#edf7ff"} anchorX="center" anchorY="middle">
        {alarm ? "ALARM" : "-60 C"}
      </Text>
      {/* handle */}
      ...
      <mesh position={[0, 1.18, 0.87]}>
        ...
        <meshStandardMaterial color="#06121f" emissive={alarm ? "#ff3b5c" : HILITE} emissiveIntensity={1.2} />
      </mesh>
      {/* feet */}
      {[-0.6, 0.6].map((x) =>
        [-0.6, 0.6].map((z) => (
